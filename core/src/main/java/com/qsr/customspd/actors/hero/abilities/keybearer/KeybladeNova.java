 ...
 * burst: every nearby enemy takes damage scaled by your sphere-grid investment, then is either
 * ignited or chilled depending on which element you've specced (dominant element, so fire and
 * frost never cancel on a single target). The grid scaling ties the ability to the build — a
...
public class KeybladeNova extends ArmorAbility {
	...
	public static int gridBonus(SphereGrid grid) {
		...
		return grid.emberLevel() + grid.frostLevel() + grid.mightLevel() + 2 * grid.abilityLevel();
	}
	...
	protected void activate(ClassArmor armor, Hero hero, Integer target) {
		...
		boolean fireDominant = grid == null || grid.emberLevel() >= grid.frostLevel();
		...
		if (hero.sprite != null && hero.sprite.parent != null) {
			for (Ballistica ray : aoe.outerRays) {
				((MagicMissile) hero.sprite.parent.recycle(MagicMissile.class)).reset(
						fireDominant ? MagicMissile.FIRE_CONE : MagicMissile.FROST_CONE,
						hero.sprite, ray.path.get(ray.dist), null);
			}
